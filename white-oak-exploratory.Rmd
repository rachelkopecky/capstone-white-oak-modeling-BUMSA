...
division_area <- read.csv("ClimateData/division_area.csv")
division_area <- division_area[,-c(2:12)]
division_area[division_area == 0] <- NA
colnames(division_area) <- c("State_code","01","02","03","04","05","06","07","08","09","10")
division_area_long <- gather(division_area, 
                        Division,
                        Percent_Ttl,
                        -State_code,
                        na.rm = TRUE)
...
pdsi_state_year <- pdsi_data_yearly %>% 
        ...
        summarise(pdsi = sum(Yearly_Avg * Percent_Ttl))
...
inventory_pdsi %>% 
        select(Abbreviation, Year, total_mortality, pdsi) %>% 
        filter(Year >= 2005) %>% 
        gather("Measure",
               "Value",
               -c(Abbreviation, Year)) %>% 
        filter(Abbreviation == 'AL') %>% 
        #filter(Measure == "total_mortality") %>% 
        ggplot(aes(x = Year, y = Value, colour = Measure, group = 1)) +
                geom_line() +
                labs(title = "Yearly Mortality and PDSI",
                     y = "Measure",
                     x = "Year") +
                theme_minimal()
...
cor(inventory_pdsi$total_inventory, inventory_pdsi$pdsi)
